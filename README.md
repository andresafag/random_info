...
| 🗄️ State Backend | S3 (with native S3 locking) |
...
### 1️⃣ Create the S3 backend bucket
...
**1. ⚙️ Lambda container architecture mismatch**
...
**8. 🏗️ Terraform `null_resource` does not re-run on code changes**
...
- ⚙️ Lambda uses `x86_64` architecture matching the `linux/amd64` Docker build platform.
